...
interface SocketData {
  range: CameraRange
  dirtyChunks: Record<ChunkName, Uint8ClampedArray>
}

const io = new Server<Client2Server, Server2Client, DefaultEventsMap, SocketData>(server, {
...
io.on('connection', socket => {
  const range = socket.data.range = socket.data.range || {
    x1: 0n,
    y1: 0n,
    x2: 0n,
    y2: 0n,
  }
  const dirtyChunks = socket.data.dirtyChunks = socket.data.dirtyChunks || Object.create(null)
  ...
  socket.on('readChunk', (x, y, callback) => {
    readChunk(BigInt(x), BigInt(y)).then(
